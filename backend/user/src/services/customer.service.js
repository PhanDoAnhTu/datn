...
class CustomerService {
  ...
  async login({ customer_email, customer_password, refreshToken = null }) {
    ...
    const match = await bcrypt.compare(
      customer_password,
      foundCustomer.customer_password
    );
    if (!match) {
    ...
    const tokens = await GenerateSignature(
      { customerId: foundCustomer._id, customer_email },
      publicKey,
      privateKey
    );
    ...
    const customer = await getIntoData({
      fileds: [
        "_id",
        "customer_name",
        "customer_email",
  ...
  async handlerRefreshToken(refreshToken) {
    ...
    if (foundToken) {
      ...
      const { customerId, customer_email } = await verifyJWT(
        refreshToken,
    ...
    if (!holderToken)
      throw new errorResponse.ForbiddenRequestError("customer not registered");
    const { customerId, customer_email } = await verifyJWT(
      refreshToken,
    ...
    const foundCustomer = await this.repository.findByEmail(customer_email);
    if (!foundCustomer)
      throw new errorResponse.ForbiddenRequestError(
        "customer not registered 2"
      );
    const tokens = await GenerateSignature(
  ...
  async checkLoginEmailTokenService({ token }) {
    try {
      ...
      if (!email || email !== user_email)
        throw new errorResponse.ErrorResponse({ message: "token not found" });

      const hasCustomer = await this.findCustomerByEmail({
      ...
      if (newCustomer) {
        ...
        //create tokenpair
        const tokens = await GenerateSignature(
          { customerId: newCustomer._id, customer_email: email },
          publicKey,
          privateKey
        );
        customer = await getIntoData({
          fileds: ["_id", "customer_name", "customer_email"],
          object: newCustomer,
        });
  ...
  async updateCustomerStatus({ customer_id, customer_status }) {
  ...
  async findcustomerByIdAndProvider({
    customer_account_id,
    customer_provider,
  }) {
    return await this.repository.findcustomerByIdAndProvider({
      customer_account_id,
      customer_provider,
    });
  }
  async newCustomerWithSocial({
    customer_account_id,
    customer_provider,
    customer_email = "",
    customer_name,
    ...
    const newCustomer = await this.repository.createCustomerWithSocial({
      customer_account_id,
      customer_provider,
      customer_avatar,
      customer_email,
  ...
  async loginWithSocial({
    ...
    try {
      const foundCustomer = await this.findcustomerByIdAndProvider({
        customer_account_id,
        customer_provider,
      });
      if (!foundCustomer) {
      ...
      const tokens = await GenerateSignature(
        { customerId: foundCustomer._id, customer_provider },
        publicKey,
        privateKey
      );
      await keyTokenService.createKeyToken({
  ...
  async changeAvatar({ customer_id, image }) {
    const updateInfo = await CustomerModel.findOneAndUpdate(
      { _id: customer_id },
      {
        $set: {
  ...
  async verifyOtp({ otp }) {
    ...
    const updatePassword = await this.changePassword({
  ...
  async resetPassword({ customer_email, customer_password }) {
    ...
    const otp = await newOtp({
      user_email: hodelCustomer.customer_email,
      user_name: hodelCustomer.customer_name,
      user_password: customer_password,
    });
  ...
  async serverRPCRequest(payload) {
    ...
    switch (type) {
      case "FIND_CUSTOMER_BY_ID_AND_PROVIDER":
        if (customer_email) {
          const foundCustomer = await this.findCustomerByEmail({
            customer_email,
          });
          console.log("email:", foundCustomer);
          if (foundCustomer) return foundCustomer;
        }
        return await this.findcustomerByIdAndProvider({
          customer_account_id,
          customer_provider,
        });
      case "NEW_CUSTOMER_WITH_SOCIAL":
        return await this.newCustomerWithSocial({
          customer_account_id,
          customer_provider,
          customer_email,
          customer_name,
