...
class AddressService {
    ...
    async CreateAddress({ customer_id, phone_number, customer_name, street, postalCode, city, country, isDefault = false }) {
    ...
    async getAddressByCustomerIdAndIsDefault({ customer_id, isDefault = true }) {
        ...
        const Addresses = await AddressModel.findOne({ customer_id: customer_id, isDefault: isDefault });
    ...
    async isDefaultAddress({ customer_id, address_id, isDefault }) {
        await AddressModel.updateOne({ customer_id: customer_id, isDefault: true }, { isDefault: false })
        await AddressModel.updateOne({ customer_id: customer_id, _id: address_id }, { isDefault: isDefault })
        return await AddressModel.find()
    }
