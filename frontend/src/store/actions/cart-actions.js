...
export const onUpdateQuantityFromCart = (data) => async (dispatch) => {
    try {
        const response = await PostData(
            '/cart/v1/updateQuantityFromCart',
            data
        );

        return dispatch({
            type: Action.UPDATE_FROM_CART,
...
export const DeleteToCartItem = (data) => async (dispatch) => {
    try {
        ...
        return dispatch({
            type: Action.DELETE_TO_CART_ITEM,
            payload: response.data,
        });
    } catch (err) {
        console.error(err);
...
export const ClearCartItems = (data) => async (dispatch) => {
    try {
        const response = await PostData(
            '/cart/v1/deleteToCartByCartIdAndUserId',
            data
        );

        return dispatch({
            type: Action.CLEAR_CART_ITEMS,
            payload: response.data,
        });
    } catch (err) {
        console.error(err);
