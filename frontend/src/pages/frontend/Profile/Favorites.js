...
import { getWishListByUserId, allProducts, deleteWishListByUserId } from '../../../store/actions';
        ...
        userInfo && (
            !wish_list
                ...
                : (
                    ...
                    wish_list.wish_list_products.length != getFavoritesFromLocalStorage().length && dispatch(getWishListByUserId({
                        userId: userInfo._id
                    }))
                )
        )
        ...
        <Tab.Panel className={'p-3 px-7'}>
            <div className="mb-4 grid gap-y-4 divide-y">
                <div className='flex justify-end'>
                    <button onClick={() => handleReload()} className="ml-3 text-white border-white border-2  px-3 py-2 font-semibold transition duration-500 ease-out hover:border-magenta-500 hover:text-magenta-500 max-sm:text-xs">
                        Làm mới
                    </button>
                    {userInfo &&
                        <button onClick={() => HandleDeleteWishList({ userId: userInfo._id })} className="ml-3 text-magenta-500 border-magenta-500 border-2  px-3 py-2 font-semibold transition duration-500 ease-out hover:border-rose-500 hover:text-rose-500 max-sm:text-xs">
