...
const TopicManagementTable = ({ searchQuery }) => {
  const TOPICS_MANAGEMENT_COLUMN_DEFS = [
    {
      title: "Tên",
  ...
  const handleChangeStatus = async (topic_id, e) => {
  ...
  const onRemove = async (topic_id, isDeleted) => {
    const id = toast.loading("Vui lòng đợi...");
    const changeStatus = await dispatch(
      isTrashTopic({ topic_id: topic_id, isDeleted: isDeleted })
    );
    if (
  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      <div className="flex flex-1 flex-col gap-[22px]">
        {width >= 768 ? (
          <StyledTable
            columns={TOPICS_MANAGEMENT_COLUMN_DEFS}
